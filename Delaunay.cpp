...
DREAL CounterClockWise(VERTEX2D_PTR pa, VERTEX2D_PTR pb, VERTEX2D_PTR pc)
{
...
DREAL InTriangle(MESH_PTR pMesh, VERTEX2D_PTR pVer, TRIANGLE_PTR pTri)
{
	...
	DREAL ccw1 = CounterClockWise(pV1, pV2, pVer);
	DREAL ccw2 = CounterClockWise(pV2, pV3, pVer);
	DREAL ccw3 = CounterClockWise(pV3, pV1, pVer);
...
void Insert(MESH_PTR pMesh, int ver_index)
{
	...
	while (pTri != NULL)
	{
		DREAL r = InTriangle(pMesh, pVer, pTri);
...
//		  FlipTest(DT, va, vd, vi)
//		  FlipTest(DT, vd, vb, vitriangle_num
...
{
	DREAL det;
	DREAL alift, blift, plift, bdxpdy, pdxbdy, pdxady, adxpdy, adxbdy, bdxady;
