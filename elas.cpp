...
void Elas::process (uint8_t* I1_,uint8_t* I2_,float* D1,float* D2,const int32_t* dims,int *tri_index)
{
  ...
  flag_image = (uint8_t*)_mm_malloc(bpl*height*sizeof(uint8_t),16);
  ...
  else 
  {
	...
	{
      memcpy(I1+v*bpl,I1_+v*dims[0],width*sizeof(uint8_t));
      memcpy(I2+v*bpl,I2_+v*dims[0],width*sizeof(uint8_t));
    }
  }
 // cout<<"bpl:"<<bpl<<endl;
	//IplImage *image_co1 = cvCreateImage(cvSize(464,375),IPL_DEPTH_8U,1);
	////int width_s = img1->widthStep;
	//for(int i = 0;i<375;i++)
	//{
	//	for(int j=0;j<bpl;j++)
	//	{
	//		uint8_t val = I1[j+i*bpl];
	//		/*image_co1->imageData[(j*3+i*image_co1->widthStep)+0] = val;
	//		image_co1->imageData[(j*3+i*image_co1->widthStep)+1] = val;
	//		image_co1->imageData[(j*3+i*image_co1->widthStep)+2] = val;	*/
	//		image_co1->imageData[j+i*image_co1->widthStep] = val;
	//	}
	//}
...
#endif
  num_cut_support = 0;
  p_support = computeSupportMatches(desc1.I_desc,desc2.I_desc);
  //computeSupportMatches_corner(desc1.I_desc,desc2.I_desc);
  //cout << "support size:"<<p_support.size();
  //cout<<"num of cut support point:"<<num_cut_support<<endl;
...
#endif
  //Éî¶È¼ÆËã
  computeDisparity(p_support,tri_1,disparity_grid_1,grid_dims,desc1.I_desc,desc2.I_desc,0,D1,tri_index);     
  computeDisparity(p_support,tri_2,disparity_grid_2,grid_dims,desc1.I_desc,desc2.I_desc,1,D2,tri_index);
  t2=clock();
...
//        free(devices);
//}
void Elas::computeSupportMatches_corner(uint8_t *I1_desc,uint8_t *I2_desc)
{
	int32_t u,v;
	int16_t d,d2;
	//vector<support_pt> p_support_corner;
	//cout<<"maxima size:"<<maxima.size()<<endl;
	for(int i = 0;i<maxima.size();i++)
	{
        ...
        // find backwards
			...
			{
				p_support.push_back(support_pt(u,v,d));
			}
		}
		else{
			num_cut_support++;
...
inline int Elas::findMatch(int32_t &u,int32_t &v,float &plane_a,float &plane_b,float &plane_c,
  ...
  if (!right_image) { 
    ...
    for (d_curr=d_plane_min; d_curr<=d_plane_max; d_curr++) {
      ...
      if (u_warp<window_size || u_warp>=width-window_size)
        continue;
      result = updatePosteriorMinimum1((__m128i*)(I2_line_addr+16*u_warp),d_curr,valid?*(P+abs(d_curr-d_plane)):0,xmm1,xmm2,val,min_val,min_d);
    }
      ...
      for (int32_t u=max((int32_t)A_u,0); u<min((int32_t)B_u,width); u++){
        if (!param.subsampling || u%2==0) {
          ...
          for (int32_t v=min(v_1,v_2); v<max(v_1,v_2); v++)
			...
			{   
				if(!right_image)
					tri_index[u+v*width] = i;
				//if((!right_image && panLL[v*width+u]==1) || (right_image && panRR[v*width+u]==1))
				{
                    flag = findMatch(u,v,plane_a,plane_b,plane_c,disparity_grid,grid_dims,I1_desc,I2_desc,P,plane_radius,valid,right_image,D);
					/*if(flag != 1)
						D[v*width + u] = 0;*/
